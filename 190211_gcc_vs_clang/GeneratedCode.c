...
void Iam_The_IntelCPU_HaHaHa(VCpuState *state, uint ip)
{
	...
	ushort *al = ((ushort *)(&eax) + 0);
	ushort *ah = ((ushort *)(&eax) + 1);
	ushort *bl = ((ushort *)(&ebx) + 0);
	ushort *bh = ((ushort *)(&ebx) + 1);
	ushort *cl = ((ushort *)(&ecx) + 0);
	ushort *ch = ((ushort *)(&ecx) + 1);
	ushort *dl = ((ushort *)(&edx) + 0);
	ushort *dh = ((ushort *)(&edx) + 1);
	const uint eiz = 0;
	char exception_string[256] = {0};
	uint exception_address = 0;
	byte *realaddr1 = null;
	uint memcache_esp_0x0_pin = 0x7fffffff;
	uint memcache_esp_0x0_data = 0xcafebeef;
	uint memcache_esp_0xc_pin = 0x7fffffff;
	uint memcache_esp_0xc_data = 0xcafebeef;
	uint memcache_esp_0x10_pin = 0x7fffffff;
	uint memcache_esp_0x10_data = 0xcafebeef;
	uint memcache_esp_0x18_pin = 0x7fffffff;
	uint memcache_esp_0x18_data = 0xcafebeef;
...
L_RET_FROM_CALL:
	switch (next_return)
	{
	case CallRetAddress__MagicReturn:
		goto L_RETURN;
	case CallRetAddress__0x80489b1:
		goto L_80489b1;
	case CallRetAddress__0x80489f5:
		goto L_80489f5;
	default:
